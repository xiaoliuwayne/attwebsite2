<template>
  <div>
    <div style="width: 50%;margin: 20px auto;display: flex">
      <el-tag class="font_2 subtitle" style="width: 145px;" >产品系列</el-tag>
        <div style="border: 1px  solid #441C08;margin: 80px 20px 20px 20px;width: 250px;border-radius:5px;" v-for="item in mainClass" @click="goSerial(item)">
          <p class="font_7" style="font-size: 15px;margin: 10px 0 0 10px">{{item.nameCN}}</p>
          <p class="font_7" style="font-size: 15px;margin: 10px 0 0 10px">{{item.nameEN}}</p>
          <hr>
    ...
    <div style="width: 50%;margin: 30px auto;">
      <p style="text-indent:2em;font-size: 16px;line-height: 1.8em" class="font_6">安腾达化工与全球知名化工企业紧密合作，致力于引进国外先进的化工材料，发掘国内优秀材料，为广大客户提供优质产品与服务。
        公司拥有多名从事橡塑行业十余年的技术工程师和技术顾问，可为客人提供专业的技术服务和解决方案，并可以提供定制化的产品。
      </p>
      <p style="font-size: 16px;line-height: 1.8em" class="font_6">服务范围:</p>
      <ol style="font-size: 16px;line-height: 1.8em" class="font_6">
    ...
    <div style="width: 50%;height:435px;margin: 50px auto;">
      <el-tag class="font_2 subtitle" style="width: 145px;">推荐产品</el-tag>
      <el-carousel :interval="4000" height="400px" style="margin-top: 20px" >
        <el-carousel-item v-for="pro in productImg" >
          <router-link :to="{name:'Detail',params:{'nameCN':pro.name}}">
            ...
            <p style="margin: 0 150px;font-size: 18px;" class="font_6">{{pro.name}}</p>
            <p style="margin: 0 150px;font-size:13px;" class="font_10" >{{pro.spec1}}</p>
            <p style="margin: 0 150px;font-size:13px;" class="font_10" >{{pro.spec2}}</p>
          </router-link>
        </el-carousel-item>
      </el-carousel>
    </div>
    <div style="width: 50%;height:90px;margin: 20px auto;">
      <el-tag class="font_2 subtitle" style="width: 205px;">产品目录下载</el-tag>
